/* Advent of Code 2024 - Day 2 Second Half */
...
void rem_lvl(int *cpy, int len, int i) 
{
        if( i == (len - 1)) return;        
        memmove(&cpy[i], &cpy[i + 1], sizeof(int) * (len - i - 1));
}

int second_pass(int *levels, int len)
{
        int arr_cpy[MAX_LEVELS];

        for (int i = 0; i < len; i++) {
                memcpy(arr_cpy, levels, sizeof(int) * MAX_LEVELS);
                rem_lvl(arr_cpy, len, i);
                
                if (is_safe(arr_cpy, len - 1))
                        return 1;
        }
...
int main(int argc, char *argv[])
{
        ...
        while (!feof(input)) {
                ...
                safe_count += is_safe(levels, arr_len) ? 1 : second_pass(levels, arr_len);
